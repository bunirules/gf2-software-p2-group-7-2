...
class Network:
    ...
    Public methods
    --------------
    get_connected_output(self, device_id, input_id): Returns the device and
    ...
    def replace_connection(self, first_device_id, first_port_id, 
        new_device_id, new_port_id, old_device_id, old_port_id):
                    Change connection from 'first device > old device' to
    ...
    def __init__(self, names, devices):
        ...
        [
            ...
            self.PORT_ABSENT_1,
            self.PORT_ABSENT_2,
            self.DEVICE_ABSENT_1,
            self.DEVICE_ABSENT_2,
            self.DEVICE_ABSENT_3,
            self.INPUT_NOT_CONNECTED,
            self.SWAP_OUTPUTS
        ] = self.names.unique_error_codes(11)

        self.error_message = {
            self.INPUT_TO_INPUT: _("Input connected to input"),
            self.OUTPUT_TO_OUTPUT: _("Output connected to output"),
            self.INPUT_CONNECTED: _("Input already connected"),
            self.INPUT_NOT_CONNECTED: _("Specified connection does not exist"),
            self.PORT_ABSENT_1: _("Specified port does not exist"),
            self.PORT_ABSENT_2: _("Specified port does not exist"),
            self.DEVICE_ABSENT_1: _("Specified device does not exist"),
            self.DEVICE_ABSENT_2: _("Specified device does not exist"),
            self.DEVICE_ABSENT_3: _("Specified device does not exist"),
            self.SWAP_OUTPUTS: _("Connections must be swapped from output to output")
        }
    ...
    def make_connection(
        ...
        if first_device is None:
            error_type = self.DEVICE_ABSENT_1

        elif second_device is None:
            error_type = self.DEVICE_ABSENT_2
            ...
            error_type = self.PORT_ABSENT_1

        return error_type

    def replace_connection(self, 
        first_device_id, first_port_id, 
        new_device_id, new_port_id, 
        old_device_id, old_port_id):
        """Change connection from 'first device > old device' to 
        'first device > new device'.

        Arguments:
            first_device_id -- device ID of the device for which the 
                connection is changing
            first_port_id -- port ID of the port for which the connection is
                changing; must be an input port of first_device
            new_device_id -- device ID of the device which will be 
                connected to first_device
            new_port_id -- port ID of the port which is will be connected to
                first_device; must be an output port of new_device
            old_device_id -- device ID of the device which is losing its
                connection to first_device
            old_port_id -- port ID of the port which is losing its connection 
                to first_device; must be an output port of old_device

        Returns:
            error_type -- error encountered in making the connection;
                returns self.NO_ERROR if there is no error
        """
        first_device = self.devices.get_device(first_device_id)
        new_device = self.devices.get_device(new_device_id)
        old_device = self.devices.get_device(old_device_id)
        ...
        if first_device is None:
            ...
            error_type = self.DEVICE_ABSENT_3
        
        elif first_port_id in first_device.inputs:
            if first_device.inputs[first_port_id] != (old_device_id, old_port_id):
                # Input is not in specified connection
                error_type = self.INPUT_NOT_CONNECTED
            elif new_port_id in new_device.inputs:
                # Both ports are inputs
                error_type = self.INPUT_TO_INPUT
            elif new_port_id in new_device.outputs:
                # Make connection
                first_device.inputs[first_port_id] = (new_device_id, new_port_id)
                error_type = self.NO_ERROR
            else:  # second_port_id is not a valid input or output port
                error_type = self.PORT_ABSENT_2

        elif first_port_id in first_device.outputs:
            # swapping the input which this output is connected to creates an invalid circuit
            error_type = self.SWAP_OUTPUTS
    ...
    def check_network(self):
        """Return unconnected inputs in the network"""
        unconnected = ""
    ...
    def execute_gate(self, device_id, x=None, y=None):
        ...
        Note: (x,y) pairs for AND, OR, NOR, NAND, NOT, XOR are: (HIGH, HIGH), (LOW,
        LOW), (LOW, HIGH), (HIGH, LOW), (HIGH, LOW),  (None, None).

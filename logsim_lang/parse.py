...
class Parser:
    ...
    def parse_network(self):
        ...
        self.error_msg += "\n\n" + _("Error Count: {}").format(self.error_count)
            ...
            if valid_device_list:
                if self.symbol.type == self.scanner.EQUALS:
                    ...
                    if self.symbol.id == self.scanner.CLOCK_ID:
                        ...
                        self.__error(
                            _("Not a supported device, supported devices: CLOCK,") +
    ...
    def __name(self, type):
        ...
        if self.symbol.type == self.scanner.NAME:
            ...
            if type == "devicename":
                self.__error(
                    _("Device names must start with a letter and be alphanumeric")
                )
            elif type == "pinname":
                self.__error(
    ...
    def __clock(self):
        ...
        if self.symbol.type == self.scanner.PARENTHESIS_LEFT:
            ...
            if self.symbol.type == self.scanner.NUMBER:
                ...
                self.__error(
                    _("Expected a number n > 0, the number of simulation cycles")+
    ...
    def __oor(self):
        ...
        if self.symbol.type == self.scanner.PARENTHESIS_LEFT:
            ...
            if self.symbol.type == self.scanner.NUMBER:
                ...
                self.__error(
                    _("Expected number of inputs for OR gate (valid range: 1-16)")
                )
        else:
            self.__error(_("Expected '('"))
    ...
    def __connectionlist(self):
        ...
        if not self.error_:
            ...
            if not unconnected == "":
                self.__error(
                    _("unconnected inputs: ") + unconnected, None, connect_Symbol
                )

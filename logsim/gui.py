...
class Gui(wx.Frame):
    ...
    def __init__(self, title, path, names, devices, network, monitors):
        ...
        self.fileMenu.Append(
            wx.ID_ABOUT, self.translator.get_translation("About")
        )
        self.fileMenu.Append(
            wx.ID_OPEN, self.translator.get_translation("Open File")
        )
        self.saveMenu = wx.Menu()
        self.saveMenu.Append(
            wx.ID_SAVE,
            self.translator.get_translation("Save Visible Plot Area"),
        )
        self.saveMenu.Append(
            wx.ID_SAVEAS, self.translator.get_translation("Save Full Plot")
        )
        self.fileMenu.AppendSubMenu(
            self.saveMenu, self.translator.get_translation("Save Image")
        )
        self.fileMenu.Append(
            wx.ID_EXIT, self.translator.get_translation("Exit")
        )
        self.menuBar.Append(
            self.fileMenu, self.translator.get_translation("File")
        )
        self.viewMenu = wx.Menu()
        self.viewMenu.Append(
            wx.ID_RESET, self.translator.get_translation("Reset View")
        )
        self.menuBar.Append(
            self.viewMenu, self.translator.get_translation("View")
        )
        self.toolsMenu = wx.Menu()
        self.toolsMenu.Append(
            wx.ID_SETUP, self.translator.get_translation("Set Language")
        )
        self.menuBar.Append(
            self.toolsMenu, self.translator.get_translation("Tools")
        )
        ...
        self.text_run = wx.StaticText(
            self,
            wx.ID_ANY,
            self.translator.get_translation("Cycles to run/continue for:"),
        )
        self.spin = wx.SpinCtrl(self, wx.ID_ANY, "10", max=1000, min=1)
        self.run_button = wx.Button(
            self, wx.ID_ANY, self.translator.get_translation("Run")
        )
        self.restart_button = wx.Button(
            self, wx.ID_ANY, self.translator.get_translation("Restart")
        )
        self.text_add_mon = wx.StaticText(
            self,
            wx.ID_ANY,
            self.translator.get_translation("Select a monitor point to add:"),
        )
        ...
        self.add_button = wx.Button(
            self, wx.ID_ANY, self.translator.get_translation("Add")
        )
        self.text_zap_mon = wx.StaticText(
            self,
            wx.ID_ANY,
            self.translator.get_translation(
                "Select a monitor point to remove:"
            ),
        )
        ...
        self.zap_button = wx.Button(
            self, wx.ID_ANY, self.translator.get_translation("Remove")
        )
        self.text_switch = wx.StaticText(
            self,
            wx.ID_ANY,
            self.translator.get_translation("Select a switch to toggle:"),
        )
        ...
        self.replace_text_1 = wx.StaticText(
            self,
            wx.ID_ANY,
            self.translator.get_translation("Replace connection:"),
        )
        ...
        self.replace_text_2 = wx.StaticText(
            self,
            wx.ID_ANY,
            self.translator.get_translation("with connection:"),
        )
        self.new_conn_box = wx.Choice(self, wx.ID_ANY, choices=[])
        self.replace_button = wx.Button(
            self, wx.ID_ANY, self.translator.get_translation("Replace")
        )
    ...
    def on_menu(self, event):
        ...
        if Id == wx.ID_EXIT:
            ...
            wx.MessageBox(
                self.translator.get_translation("Logic Simulator\nCreated by")
                + " Robbie Hodgeon, Scott Irvine "
                + self.translator.get_translation("and")
                + " Sahil Sindhi\n2022",
                self.translator.get_translation("About Logsim"),
    ...
    def on_replace_button(self, event):
            ...
            if self.output_for_swap is not None:
                ...
                if error == self.network.NO_ERROR:
                    message += (
                        ...
                        + self.translator.get_translation(" to ")
                    ...
                    message += self.translator.get_translation(
        ...
        if undo:  # if the connection was successfully replaced
            ...
            swap_conn_dialog.SetOKCancelLabels(
                self.translator.get_translation("OK"),
                self.translator.get_translation("Undo"),
            )
            ...
            if action == wx.ID_CANCEL:
                ...
                undo_dialog = wx.MessageDialog(
                    self,
                    message,
                    self.translator.get_translation("Undo status message"),
                    wx.OK,
                )
            ...
            swap_conn_dialog = wx.MessageDialog(
                self,
                message,
                self.translator.get_translation("Connection swap failed!"),
                wx.OK,
            )
    ...
    def on_open_file(self, file=True):
        ...
        if action == wx.ID_OK:  # if file selected
            ...
            if Path(path).suffix == ".txt":
                ...
                parser = Parser(
                    names, devices, network, monitors, scanner, self.translator
                )
                ...
                if file is False:
                    ...
                    wx.MessageBox(
                        ...
                        self.translator.get_translation("Error opening file"),
                        wx.ICON_ERROR | wx.OK,
                    )
                    dialog.Destroy()
            ...
            if (
                wx.MessageBox(
                    ...
                    self.translator.get_translation("File required"),
                    wx.ICON_ERROR | wx.YES_NO,
                )
                == wx.YES
            ):
    ...
    def on_save_file(self, full=False):
        ...
        dialog = wx.FileDialog(
            self,
            message=self.translator.get_translation("Save file as ..."),
            defaultDir=str(Path.cwd()),
            defaultFile="",
        ...
        if dialog.ShowModal() == wx.ID_OK:  # if save requested
            ...
            if Path(path).suffix == ".png":  # make sure file type is png
                ...
                wx.MessageBox(
                    ...
                    self.translator.get_translation("Error saving file"),
                    wx.ICON_ERROR | wx.OK,
                )
        dialog.Destroy()
    ...
    def choose_language(self):
        """Creates dialog box to allow language choice."""
        dialog = wx.SingleChoiceDialog(
            self,
            self.translator.get_translation("Choose your language:"),
            "",
            list(self.translator.lang_options.keys()),
            wx.OK & (~wx.CLOSE) & (~wx.MINIMIZE_BOX),
        )
        dialog.SetSize(300, 300)
        if dialog.ShowModal() == wx.ID_OK:  # once language selected
            language = dialog.GetStringSelection()
            self.translator.lang = self.translator.lang_options[language]
            dialog.Destroy()
            wx.MessageBox(
                self.translator.get_translation("Language now set to")
                + f" {language}",
                self.translator.get_translation(
            ...
            self.fileMenu.SetLabel(
                wx.ID_ABOUT, self.translator.get_translation("About")
            )
            self.fileMenu.SetLabel(
                wx.ID_OPEN, self.translator.get_translation("Open File")
            )
            self.saveMenu.SetLabel(
                wx.ID_SAVE,
                self.translator.get_translation("Save Visible Plot Area"),
            )
            self.saveMenu.SetLabel(
            ...
            self.saveMenu.SetTitle(
                self.translator.get_translation("Save Image")
            )
            self.fileMenu.SetLabel(
                wx.ID_EXIT, self.translator.get_translation("Exit")
            )
            self.fileMenu.SetTitle(self.translator.get_translation("File"))
            self.viewMenu.SetLabel(
                wx.ID_RESET, self.translator.get_translation("Reset View")
            )
            self.viewMenu.SetTitle(self.translator.get_translation("View"))
            self.toolsMenu.SetLabel(
                wx.ID_SETUP, self.translator.get_translation("Set Language")
            )
            self.toolsMenu.SetTitle(self.translator.get_translation("Tools"))

            # Configure the widgets
            self.text_run.SetLabel(
                self.translator.get_translation("Cycles to run/continue for:"),
            )
            self.run_button.SetLabel(self.translator.get_translation("Run"))
            self.restart_button.SetLabel(
                self.translator.get_translation("Restart")
            )
            self.text_add_mon.SetLabel(
                self.translator.get_translation(
                    "Select a monitor point to add:"
                ),
            )
            self.add_button.SetLabel(self.translator.get_translation("Add"))
            self.text_zap_mon.SetLabel(
                self.translator.get_translation(
                    "Select a monitor point to remove:"
                ),
            )
            self.zap_button.SetLabel(self.translator.get_translation("Remove"))
            self.text_switch.SetLabel(
                self.translator.get_translation("Select a switch to toggle:"),
            )
            self.replace_text_1.SetLabel(
                self.translator.get_translation("Replace connection:"),
            )
            self.replace_text_2.SetLabel(
                self.translator.get_translation("with connection:"),
            )
            self.replace_button.SetLabel(
                self.translator.get_translation("Replace")
            )

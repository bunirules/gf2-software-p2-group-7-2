...
class Parser:
    ...
    __error(self, message, stopping_symbol=";", symbol=None):
    ...
    def parse_network(self):
        ...
        if (
            ...
            self.__error(_("Expected 'END'"))
        self.error_msg += "\n\n" + _("Error Count: {}").format(
            self.error_count
        )
        print(_("Error Count: {}").format(self.error_count))
    ...
    def __circuit(self):
        ...
        if (
            ...
            self.__error(_("Expected 'CIRCUIT'"), "{")
        if self.symbol.type == self.scanner.BRACE_LEFT:
            self.symbol = self.scanner.get_symbol()
        else:
            self.__error(_("Expected '{'"), None)
    ...
    def __devicelist(self):
        ...
        if (
            ...
            self.__error(_("Expected 'DEVICES'"), "{")
        if self.symbol.type == self.scanner.BRACE_LEFT:
            self.symbol = self.scanner.get_symbol()
        else:
            self.__error(_("Expected '{'"), None)
        ...
        while (
            ...
            if self.symbol.id == self.scanner.CONNECT_ID:
                ...
                # Or used CONNECT as name
                    ...
                    self.__error(
                        _("Device names cannot be Keywords: 'CIRCUIT',")
                        + " 'DEVICES', 'CONNECT', 'MONITOR', 'END','CLOCK',"
                        " 'SWITCH', 'AND', 'NAND', 'OR', 'NOR', 'XOR', 'NOT'"
                        " 'DTYPE'"
                    )
                    self.symbol = self.scanner.get_symbol()
            ...
            if valid_device_list:
                if self.symbol.type == self.scanner.EQUALS:
                    ...
                    if self.symbol.id == self.scanner.CLOCK_ID:
                        ...
                        self.__error(
                            _(
                                "Not a supported device, supported devices:"
                                " CLOCK,"
                            )
                            + " SWITCH, AND, NAND, OR, NOR, DTYPE, XOR, NOT"
                        )
                else:
                    self.__error(_("Expected '=' or ','"))
    ...
    def __name(self, type):
        ...
        if self.symbol.type == self.scanner.NAME:
            ...
            self.__error(
                _("Names cannot be Keywords: 'CIRCUIT', 'DEVICES', 'CONNECT',")
            ...
            if type == "devicename":
                self.__error(
                    _(
                        "Device names must start with a letter and be"
                        " alphanumeric"
                    )
                )
            elif type == "pinname":
                self.__error(
                    _("Pin names must start with a letter and be alphanumeric")
                )
            return False
    ...
    def __clock(self):
        ...
        if self.symbol.type == self.scanner.PARENTHESIS_LEFT:
            ...
            if self.symbol.type == self.scanner.NUMBER:
                ...
                if num <= 0:
                    self.__error(_("Clock half period must be greater than 0"))
                    self.skip = True
                if not self.error_:
                ...
                self.__error(
                    _(
                        "Expected a number n > 0, the number of simulation"
                        " cycles"
                    )
                    + _(" after which the state changes")
                )
        else:
            self.__error(_("Expected '('"))
    ...
    def __switch(self):
        ...
        if self.symbol.type == self.scanner.PARENTHESIS_LEFT:
            ...
            if self.symbol.id == 0 or self.symbol.id == 1:
                ...
                self.__error(_("Expected state, either 0(OFF) or 1(ON)"))
        else:
            self.__error(_("Expected '('"))
        ...
        if self.symbol.type == self.scanner.PARENTHESIS_LEFT:
            ...
            if self.symbol.type == self.scanner.NUMBER:
                ...
                if not (0 < num and num <= 16):
                    self.skip = True
                    self.__error(_("Number of inputs must be between 1-16"))
                if not self.error_:
                    self.cur_attribute = num
                if not self.skip:
                    self.symbol = self.scanner.get_symbol()
                ...
                self.__error(
                    _("Expected number of inputs for AND gate (valid range:")
                    + " 1-16)"
                )
        else:
            self.__error(_("Expected '('"))
    ...
    def __nand(self):
        ...
        if self.symbol.type == self.scanner.PARENTHESIS_LEFT:
            ...
            if self.symbol.type == self.scanner.NUMBER:
                ...
                self.__error(
                    _("Expected number of inputs for NAND gate (valid range:")
                    + " 1-16)"
                )
        else:
            self.__error(_("Expected '('"))
    ...
    def __nor(self):
        ...
        if self.symbol.type == self.scanner.PARENTHESIS_LEFT:
            ...
            if self.symbol.type == self.scanner.NUMBER:
                ...
                self.__error(
                    _("Expected number of inputs for NOR gate (valid range:")
                    + " 1-16)"
                )
        else:
            self.__error(_("Expected '('"))
    ...
    def __oor(self):
        ...
        if self.symbol.type == self.scanner.PARENTHESIS_LEFT:
            ...
            if self.symbol.type == self.scanner.NUMBER:
                ...
                self.__error(
                    _(
                        "Expected number of inputs for OR gate (valid range:"
                        " 1-16)"
                    )
                )
        else:
    ...
    def __connectionlist(self):
        ...
        if not self.skip:
            if (
                ...
                self.__error(_("Expected 'CONNECT'"), "{")
        self.skip = False
        ...
        if not self.error_:
            ...
            if not unconnected == "":
                self.__error(
                    _("unconnected inputs: ") + unconnected,
                    None,
                    connect_Symbol,
                )
    ...
    def __con(self):
        ...
        if not self.error_:
            ...
            cur_in_pin_ids = [
                self.__names.query(dev.string)
                for dev in self.cur_pin_name_list
            ]
    ...
    def __monitorlist(self):
        ...
        if not self.skip:
            if (
                ...
                self.__error(_("Expected 'MONITOR'"), "{")
        self.skip = False

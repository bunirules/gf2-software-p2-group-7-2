...
class MyGLCanvas(wxcanvas.GLCanvas):
    ...
    render(self, names=None, signals=None, swap=True):
    ...
    set_max_dims(self, names, signals): Find dimensions of full plot.

    limit_pan(self): Limits x and y positions to sensible range.

    set_view(self, x=0, y=0, zoom=1): Set view to given position.

    save_image(self, full): Get image data from canvas.
    """
...
class Gui(wx.Frame):
    ...
    draw_signals(self, monitors_dict): Draws monitored signals on canvas.

    run_network(self, cycles): Run the network for the specified number
    ...
    on_restart_button(self, event): Handle the event when the user clicks
                                        the restart button.

    on_add_box(self, event): Handle the event when user selects a monitor
                                to add from the list.

    on_zap_box(self, event): Handle the event when user selects a monitor
                                to zap from the list.

    on_switch_box(self, event): Handle the event when user selects a switch
                                to toggle from the box.

    on_add_button(self, event): Handle the event when the user presses the
                                add monitor button.

    on_zap_button(self, event): Handle the event when the user presses the
                                remove monitor button.

    on_toggle_0(self, event): Handle the event when the user presses the
                                toggle_0 button.

    on_toggle_1(self, event): Handle the event when the user presses the
                                toggle_1 button.

    on_swap_conn(self, event): Handle the event when the user selects a
                                connection to swap.

    on_new_conn(self, event): Handle the event when the user selects a new
                                connection.

    on_replace_button(self, event): Handle the event when the user presses
                                        the replace button.

    on_horizontal_scrollbar(self, event): Change canvas position after
                                            horizontal scrollbar is moved.

    on_vertical_scrollbar(self, event): Change canvas position after vertical
                                            scrollbar is moved.

    on_open_file(self, file=True): Create and show the Open FileDialog.

    on_save_file(self, full=False): Create and show the Save FileDialog.

    get_input_names(self): Return a list of all input names in the network.

    get_conn_names(self): Return a list with the names of each connection
                            in the network.

    get_new_conn_options(self, input_name): Return list of possible new
                                            connections for user to select.

    new_file(self): Set up GUI for new file.

    set_choices(self, choicebox, choices): Set choices in dropdowns.
    """
    ...
    def __init__(self, title, path, names, devices, network, monitors):
        ...
        self.fileMenu.Append(wx.ID_ABOUT, _(_("About")))
        self.fileMenu.Append(wx.ID_OPEN, _(_("Open File")))
        self.saveMenu = wx.Menu()
        self.saveMenu.Append(
            wx.ID_SAVE,
            _(_("Save Visible Plot Area")),
        )
        self.saveMenu.Append(wx.ID_SAVEAS, _(_("Save Full Plot")))
        self.fileMenu.AppendSubMenu(self.saveMenu, _(_("Save Image")))
        self.fileMenu.Append(wx.ID_EXIT, _(_("Exit")))
        self.menuBar.Append(self.fileMenu, _(_("File")))
        self.viewMenu = wx.Menu()
        self.viewMenu.Append(wx.ID_RESET, _(_("Reset View")))
        self.menuBar.Append(self.viewMenu, _(_("View")))
        ...
        self.run_button = wx.Button(self, wx.ID_ANY, _(_("Run")))
        self.restart_button = wx.Button(self, wx.ID_ANY, _(_("Restart")))
        ...
        self.add_button = wx.Button(self, wx.ID_ANY, _(_("Add")))
        self.text_zap_mon = wx.StaticText(
            self,
            wx.ID_ANY,
            _(_("Select a monitor point to remove:")),
        )
        ...
        self.zap_button = wx.Button(self, wx.ID_ANY, _(_("Remove")))
        ...
        self.replace_button = wx.Button(self, wx.ID_ANY, _(_("Replace")))
    ...
    def on_add_box(self, event):
        """Handle the event when user selects a monitor to add from the list.
    ...
    def on_zap_box(self, event):
        """Handle the event when user selects a monitor to zap from the list.
    ...
    def on_swap_conn(self, event):
        """Handle the event when the user selects a connection to swap."""
    ...
    def on_new_conn(self, event):
        """Handle the event when the user selects a new connection."""
    ...
    def on_replace_button(self, event):
        """Handle the event when the user presses the replace button."""
        message = ""
        undo = False
        if self.input_for_swap is not None:
            if self.output_for_swap is not None:
                ...
                if error == self.network.NO_ERROR:
                    message += (
                        _("Successfully swapped connection into ")
            ...
            message += _("Please select an existing connection to replace.")
        if undo:  # if the connection was successfully replaced
            ...
            swap_conn_dialog = wx.MessageDialog(
                self,
                message,
                _("Connection swapped successfully"),
                wx.OK | wx.CANCEL,
            )
            ...
            if action == wx.ID_CANCEL:
                ...
                if error == self.network.NO_ERROR:
                    message += _("Successfully undid connection swap.\n")
                else:
                    message += _(
    ...
    def on_open_file(self, file=True):
        ...
        dialog = wx.FileDialog(
            self,
            message=_("Choose a circuit definition file"),
        ...
        if action == wx.ID_OK:  # if file selected
            path = dialog.GetPath()
            print(_("You chose the following file:") + f' "{path}"')
    ...
    def on_save_file(self, full=False):
        ...
        if dialog.ShowModal() == wx.ID_OK:  # if save requested
            path = dialog.GetPath()
            print(_("You chose the following filename:") + f' "{path}"')
            image = self.canvas.save_image(full)
            if Path(path).suffix == ".png":  # make sure file type is png
                image.save(path, "PNG")
            else:
                wx.MessageBox(
                    _("Could not save image! File type must be PNG."),
                    _("Error saving file"),
                    wx.ICON_ERROR | wx.OK,
                )
        dialog.Destroy()
    ...
    def get_input_names(self):
        """Return a list of all input names in the network.
    ...
    def get_new_conn_options(self, input_name):
        """Return list of possible new connections for user to select.

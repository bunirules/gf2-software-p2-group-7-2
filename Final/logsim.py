...
def main(arg_list):
    ...
    for option, path in options:
        if option == "-h":  # print the usage message
            ...
            locale.AddCatalog("logsim_es.mo")
    ...
    if not options:  # no option given, use the graphical user interface
        ...
        if len(arguments) == 0:  # no argument or language choice
            ...
            monitors = Monitors(names, devices, network)
            gui = Gui(
                _("Logic Simulator"), "", names, devices, network, monitors
            )
            app.MainLoop()
        elif len(arguments) > 0:  # too many arguments
            print(_("Error: When using GUI, do not give any arguments.\n"))
            print(usage_message)
            sys.exit()
